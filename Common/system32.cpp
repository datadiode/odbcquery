 ...
 * Added a few things to support ODBCQuery System32 build configuration.
 */
...
typedef void (__cdecl *_se_translator_function)(unsigned int, EXCEPTION_POINTERS *);
...
void __stdcall eh_vec_dtor(void *p, unsigned int size, int count, void (__thiscall*dtor)(void *))
{
	p = static_cast<char *>(p) + count * size;
	while (count > 0)
	{
		--count;
		p = static_cast<char *>(p) - size;
